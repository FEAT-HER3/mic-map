/**
 * @file config_manager_impl.cpp
 * @brief P8 D-02: ConfigManagerImpl relocated from src/core/src/config_manager.cpp
 *        to keep micmap_core JSON-free (AssertNoJsonInCore 4-root scope per D-02).
 *        Body lifted verbatim - no behavioral change.
 *
 *        IConfigManager interface + the createConfigManager() factory DECLARATION
 *        stay in src/core/include/micmap/core/config_manager.hpp (header is JSON-
 *        free already - only filesystem + optional + chrono). The full impl class
 *        + factory implementation live HERE in apps/micmap so micmap_core has zero
 *        nlohmann/json dependency.
 *
 *        mic_test never calls createConfigManager (verified - apps/mic_test/main.cpp
 *        does not reference IConfigManager) so the relocation is invisible to it
 *        and the LIB-02 headless invariant is preserved trivially.
 */
