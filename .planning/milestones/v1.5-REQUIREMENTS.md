# Milestone v1.5 Requirements Archive — Seamless SteamVR Integration

**Status:** ✅ SHIPPED 2026-04-24
**Defined:** 2026-04-22
**Closed:** 2026-04-29
**Coverage:** 29 / 31 satisfied; 2 deferred (DOC-01, DOC-02 → next milestone)
**Audit:** [v1.5-MILESTONE-AUDIT.md](v1.5-MILESTONE-AUDIT.md)

> Frozen snapshot of v1 requirements at milestone close. Source of truth for what shipped — checkbox state reconciled against UAT/VERIFICATION evidence (audit pass overrides stale `[ ]`). Active requirements for the next milestone live in `.planning/REQUIREMENTS.md` (rebuilt by `/gsd-new-milestone`).
...
- [x] **CFG-01**: `ConfigManager::loadDefault()` parses `%APPDATA%/MicMap/config.json` via nlohmann/json on startup — replaces the stub at `src/core/src/config_manager.cpp:142`
- [x] **CFG-02**: Malformed or corrupted config triggers a `json::parse_error` catch, backs the file up to `config.json.corrupted.YYYYMMDD-HHMMSS`, and falls back to defaults — no crash, no data loss, visible log line
- [x] **CFG-03**: Each field is read with `.value(key, default)` or equivalent guarded accessor and then bounds-validated (detection duration, sensitivity, sample rate) with clamp-to-range + warning log on out-of-range values
- [x] **CFG-04**: Write/read round-trip is identity — a config written by `saveDefault()` and immediately reloaded produces identical in-memory state (write path migrates to nlohmann/json if necessary for parity)
- [x] **CFG-05**: User settings that persist: selected audio device, detection duration (ms), sensitivity, SteamVR options. Training data path unchanged.
...
- [x] **INST-01**: `installer/MicMap.iss` produces a single admin-elevated Setup `.exe` via Inno Setup 6.7.1 with a stable `AppId` GUID that supports upgrade-in-place across versions
- [x] **INST-02**: Installer detects running `vrserver.exe` / `vrmonitor.exe` / `vrcompositor.exe` / `vrdashboard.exe` / `vrwebhelper.exe` via WMI and prompts the user to close SteamVR before proceeding; DLLs use `restartreplace` as defense-in-depth
- [x] **INST-03**: Installer runs `vrpathreg.exe removedriver <driver-dir>` unconditionally before `vrpathreg.exe adddriver <driver-dir>` to prevent duplicate entries (OpenVR issue #1653); `vrpathreg.exe` presence is gated by `FileExists` so missing SteamVR is a clean skip
- [x] **INST-04**: Installer runs `micmap.exe --register-vrmanifest` as a post-install `[Run]` step to register the vrmanifest and enable auto-launch
- [x] **INST-05**: Uninstaller `[UninstallRun]` invokes `micmap.exe --unregister-vrmanifest` and `vrpathreg.exe removedriver <driver-dir>` for symmetric teardown; `Uninstallable=yes` in the script (unlike bey-closer-t1's nested install)
- [x] **INST-06**: Upgrade from legacy 0.x (virtual-controller) version cleans up stale controller bindings under `%LOCALAPPDATA%\openvr\input\` so no ghost controller haunts SteamVR after upgrade
- [x] **INST-07**: CMake `package` target invokes ISCC.exe with the correct `/D` defines so `cmake --build --target package` produces `MicMap-Setup-vX.Y.Z.exe`
- [x] **INST-08**: Installer patches `<SteamVR>/resources/config/vrcompositor_bindings_generic_hmd.json` at install time to wire `/user/head/input/system` -> `ToggleDashboard` / `ToggleRoomView` + lasermouse `LeftClick` / `Pointer` (mirrors the discovery in `driver/src/bindings_patcher.cpp`). Saves the original alongside as `.micmap_backup` on first write. Uninstaller restores from backup when present. Driver-side patcher remains as fallback for manual driver installs.
...
|-------------|-------|--------|
| SVR-01 | Phase 1 — Driver Sidecar Migration | ✓ Complete (live-UAT — driver loads with no virtual controller, no laser beam; Phase 04 UAT 6) |
| SVR-02 | Phase 1 — Driver Sidecar Migration | ✓ Complete (HMD-container deferred component creation; Phase 03 5-cycle UAT) |
| SVR-03 | Phase 1 — Driver Sidecar Migration | ✓ Complete (`MicMap: /input/system/click created (handle=7)` in vrserver.txt; Phase 04 UAT 6) |
| SVR-04 | Phase 1 — Driver Sidecar Migration | ✓ Complete (HMD deactivate/reactivate handled via `VREvent_TrackedDeviceDeactivated` + RunFrame re-creation) |
| SVR-05 | Phase 1 — Driver Sidecar Migration | ✓ Complete (`test_command_queue` GREEN; HTTP→RunFrame thread separation in driver) |
| SVR-06 | Phase 1 — Driver Sidecar Migration | ✓ Complete (`IDriverClient::tap()` single-tap collapse, commit `10112ba`) |
| SVR-07 | Phase 1 — Driver Sidecar Migration | ✓ Complete (forbidden-string grep gate green at Plan 01-04 close; `-Werror`/`/WX` clean) |
| SVR-08 | Phase 1 — Driver Sidecar Migration | ✓ Complete (dashboard_manager TU dropped from `micmap_steamvr.lib`) |
| SVR-09 | Phase 1 — Driver Sidecar Migration | ✓ Complete (single tap endpoint `POST /button {"kind":"tap"}`; commit `10112ba`) |
| SVR-10 | Phase 1 — Driver Sidecar Migration | ✓ Complete (DriverLog init line confirmed in vrserver.txt; Phase 04 UAT) |
| SVR-11 | Phase 1 — Driver Sidecar Migration | ✓ Complete (live HMD UAT — `hmd_button_test` + Bigscreen Beyond: dashboard opens, no laser, post-sleep trigger works) |
| CFG-01 | Phase 2 — Config Read-Back | ✓ Complete (M-1 manual cycle PASSED 2026-04-23 post startup-fix `73681c5`; Phase 04 UAT 6 implicitly re-confirms) |
| CFG-02 | Phase 2 — Config Read-Back | ✓ Complete (`test_config_manager` T-2 GREEN; backup + retention verified) |
| CFG-03 | Phase 2 — Config Read-Back | ✓ Complete (T-3 GREEN; clamp + pow2-snap on all bounds-checked fields) |
| CFG-04 | Phase 2 — Config Read-Back | ✓ Complete (T-1 GREEN; Unicode wstring + UNC + timestamp round-trip identity) |
| CFG-05 | Phase 2 — Config Read-Back | ✓ Complete (M-1 PASSED post-startup-fix; covered in Phase 04 UAT 6 auto-launch + tray) |
...
| INST-01 | Phase 4 — Installer | ✓ Complete (UAT 8: Add/Remove single entry + frozen AppId GUID; vrpathreg single entry across upgrade) |
| INST-02 | Phase 4 — Installer | ✓ Complete (UAT 3: WMI gate fires after WQL single-quote + tasklist fallback fix `c7b96dd`) |
| INST-03 | Phase 4 — Installer | ✓ Complete (UAT 5+8: removedriver-before-adddriver; single vrpathreg entry across upgrade) |
| INST-04 | Phase 4 — Installer | ✓ Complete (UAT 5+6: `--register-vrmanifest` succeeds, MicMap auto-launches) |
| INST-05 | Phase 4 — Installer | ✓ Complete (HR-01 + MR-01 fixed; UninstallSilent fix; UAT 9+10 — vrpathreg clean post-uninstall, .micmap_backup restored) |
| INST-06 | Phase 4 — Installer | ✓ Complete (voided per D-07 — 0.x never shipped; SweepLegacyBindings as defense-in-depth) |
| INST-07 | Phase 4 — Installer | ✓ Complete (UAT 1: `build/installer/MicMap-Setup-v0.1.0.exe` produced ~3.3 MB post-runtime-DLL bundling) |
| INST-08 | Phase 4 — Installer | ✓ Complete (UAT 5: bindings patched + .micmap_backup + lighthouse_hmd_profile.json marker; UAT 10: bindings restored on uninstall) |
| DOC-01 | Phase 5 — Documentation | ⏭ Deferred to next milestone (Phase 05 carried forward per STATE.md 2026-04-24) |
| DOC-02 | Phase 5 — Documentation | ⏭ Deferred to next milestone (Phase 05 carried forward per STATE.md 2026-04-24) |

**Coverage:**
- v1 requirements: 31 total
- Satisfied (in-scope): 29 / 29
- Deferred (intentional scope reduction at release): 2 (DOC-01, DOC-02)
- Unmapped: 0

---
*Requirements defined: 2026-04-22*
*Milestone closed: 2026-04-29 — released as v1.5 (pre-GSD baseline was v1.0.0)*
*Audit: see `v1.5-MILESTONE-AUDIT.md` (`tech_debt` — bookkeeping drift only, no behavioral blockers)*
