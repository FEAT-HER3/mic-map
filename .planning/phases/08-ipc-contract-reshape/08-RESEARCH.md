...
**Phase requirements covered:** IPC-01, IPC-02, IPC-03, IPC-04, IPC-05, IPC-07, IPC-08, LIB-04, HEALTH-01..07. (IPC-06 — driver as sole writer of `training_data.bin` — is mapped to P8 in REQUIREMENTS.md but covered there only by inheritance via the same `ReplaceFileW` helper; the actual training-data ownership transfer ships in P9 per CONTEXT.md `<deferred>`.)
...
| IPC-02 | `GET /telemetry/level` returns current RMS/dBFS for 5–10 Hz polling, 250 ms client timeout | `std::atomic<float>` on AudioWorker State (extending the existing `sample_rate` atomic shape at `audio_worker.hpp:124`). Lock-free read from HTTP thread. RMS already computed by audio cb under `MICMAP_DEBUG_RMS_LOG`; lift the computation out of the ifdef and store it. |
| IPC-03 | `GET /devices` returns enumerated WASAPI capture devices (id, friendly name, default flag) | Reuse `IAudioCapture::enumerateDevices()` already invoked by `audio_worker.cpp:221`. Wrap result in a 1 s TTL cache (D-17). Invalidate on `IMMNotificationClient` add/remove (Pitfall 13 alive-flag already in place). |
| IPC-04 | `GET /settings` (current AppConfig); `PUT /settings` (full AppConfig, atomic validate → apply → persist; HTTP 400 with `{"field","reason"}` on rejection) | Hand-rolled validator (D-15) with first-failed-field semantics. Validation order: schema → range → commit (Pitfall 7.3). Persist via the existing `writeAtomicWindows` helper at `config_manager.cpp:322` — lifted into a driver-callable `config_io.cpp` (D-14). |
...
| IPC-07 | All endpoints bind `127.0.0.1` only | `HttpServer` ctor at `http_server.hpp:64` already takes `host="127.0.0.1"`. Verified by `netstat -an` UAT step (D-27.5). Add lint enforcing the literal. |
| IPC-08 | HTTP-thread → CommandQueue → RunFrame v1.5 SVR-05 boundary survives | New routes never call `vr::*` and never `commandQueue_->push(...)`. Reads = atomic-snapshot loads; writes = atomic mutations + ReplaceFileW. Verifiable by lint similar to `AssertDetectionRunnerNoVrApi.cmake`. |
| LIB-04 | Logger sinks injected at construction; no `#ifdef MICMAP_DRIVER_BUILD` inside `micmap_core_runtime` | Existing `Logger::setLogger(std::shared_ptr<ILogger>)` shape unchanged. New sinks: `FileLogSink`, `DriverLogSink`, `StdoutLogSink`, `MultiSinkLogger`. Composition root pattern (D-21). |
| HEALTH-01 | Driver-loaded indicator (red on ECONNREFUSED, green on success) | Existing `DriverClient::connect()` already uses ECONNREFUSED-shaped failure path (`vr_input.cpp:147`). Surface as a single bool published from the 1 Hz `/health` poll. |
...
| HEALTH-04 | Last-trigger relative timestamp ("3 s ago", "—") | Add `last_trigger_at` (UTC) to `DriverState` snapshot. Updated by DetectionRunner on rising-edge Triggered (right next to existing `triggers_.fetch_add(1)` in detection_runner). |
...
| AppConfig JSON ADL hooks (`to_json`/`from_json`) | Driver / `driver/src/config_io.cpp` | Client / `apps/micmap/main.cpp` (PUT body construction) | Both binaries serialize/deserialize, but each privately compiles the ADL hooks. mic_test stays json-free. |
...
| Client driver-loaded indicator | Client / `apps/micmap/main.cpp` (extends existing `drvOk` at `main.cpp:542`) | — | ECONNREFUSED is the canonical signal; surface in red/green dot. |
...
**When to use:** AppConfig (D-09), DriverState (state pill + last_trigger_at + device_id + device_state + last_error). Already proven in P7 for `DetectionConfig` (`detection_runner.hpp:120-121`).
...
public:
    ...
    bool applyValidatedConfig(core::AppConfig candidate) {
        ...
        return saveConfigJson(getConfigPath(), *next);  // ReplaceFileW
    }
...
|---------|-------------|-------------|-----|
| HTTP server / client | Custom socket + parser | cpp-httplib v0.20.1 (already locked) | Already in tree; CVE-fixed; same wire format as v1.5. |
...
| Atomic file replace | Custom `MoveFileExW` dance | Existing `writeAtomicWindows` at `src/core/src/config_manager.cpp:322` | v1.5 CFG-04 already battle-tested; lift into `driver/src/config_io.cpp`. |
| Corruption backup with rotation | Custom timestamp scheme | Existing `backupAndRotate` at `src/core/src/config_manager.cpp:280` | v1.5 CFG-05 already retains 5 generations; reuse verbatim. |
| WASAPI device enumeration | Re-roll `IMMDeviceEnumerator` walk | Existing `IAudioCapture::enumerateDevices()` at `audio_worker.cpp:221` | Already used by AudioWorker; trivially reused for `GET /devices`. |
| RMS computation | Re-roll | Existing audio cb at `audio_worker.cpp:317-324` (under `MICMAP_DEBUG_RMS_LOG`) | Lift the four-line `sumSq → sqrt` block out of the ifdef and store result in a `std::atomic<float>` on State. |
...
| `vr::VRDriverLog` Rule-3 guard | Re-roll Rule-3 logic | Existing `SafeDriverLog` from P6 (`driver_log.hpp`) | Already wraps `vr::VRDriverContext()` null-check; wrap *that* in `DriverLogSink::log(...)`. |
...
**What goes wrong:** `Logger::setLogger()` called inside `DeviceProvider::Init` AFTER `httpServer_->Start()`. HTTP server logs its "starting" message via the default `ConsoleLogger` (or whatever `Logger::logger_` defaulted to in `logger.cpp:63`), bypassing the new file sink.

**Why it happens:** `Logger::logger_` is statically initialized to `std::make_shared<ConsoleLogger>()` (`logger.cpp:63`). Until `setLogger()` runs, all calls flow there.
...
### Pitfall 6: ECONNREFUSED-vs-timeout misclassification
...
**Why it happens:** cpp-httplib's `httplib::Client::Get()` returns `nullptr` on connect-refused, on timeout, on DNS failure — all collapsed into one signal.
...
**What goes wrong:** Mirror of Pitfall 3 on the client side. `WinMain` runs early `MICMAP_LOG_*` lines (e.g., `MicMapApp::initialize()` at `main.cpp:220` runs before sink wiring), which flow to the default `ConsoleLogger` and never reach `micmap.log`.
...
**How to avoid:** Run `grep -rn 'vr_input\|IDriverClient\|driver_client\|DriverClient' src/ apps/ tests/ driver/` after the rename plan and fix every hit. Comment-only references are fine to fix lazily; symbol/name references must update.
...
void from_json(const nlohmann::json& j, AudioConfig& c) {
    ...
    // behavior at src/core/src/config_manager.cpp:120-131.
...
### Example 2: PUT /settings handler (skeleton)
...
server_->Put("/settings", [this](const httplib::Request& req, httplib::Response& res) {
    ...
    // 4. Persist FIRST (Pitfall 2 — option A)
...
#include "driver_log.hpp"           // existing SafeDriverLog wrapper
...
| cpp-httplib v0.14.3 | cpp-httplib v0.20.1 (CVE-2025-46728 fixed) [VERIFIED: github.com/yhirose/cpp-httplib v0.20.1 tag exists] | P8 plan 08-00 | Wire format compatible; documented bump. |
...
- cpp-httplib v0.43.3 is the current upstream release [VERIFIED: webfetch on github.com/yhirose/cpp-httplib/releases, 2026-05-04]. CONTEXT D-05 explicitly chooses v0.20.1 (the minimum CVE-fixed version) to minimize wire-format / API drift risk. Document the version pin and CVE rationale in PLAN; revisit in a future deps-refresh phase.
...
| A2 | The existing audio cb at `audio_worker.cpp:317-324` can be lifted out of the `MICMAP_DEBUG_RMS_LOG` ifdef without regressing performance — RMS is already computed today in the debug build, just not stored | IPC-02 telemetry | Verifiable by quick local profile (FFT inside the audio cb is the dominant cost per CONCERNS.md backlog item; another sumSq-and-sqrt is negligible). |
...
| A5 | The existing `writeAtomicWindows` helper at `config_manager.cpp:322` can be lifted into a driver TU with no shared-lib changes (the helper already has no shared-lib state besides the path argument) | Persist plumbing for PUT /settings | Read of source confirms it's a free function inside an anonymous namespace; copy-paste is safe but should be refactored to a header to avoid maintenance drift. |
| A6 | `cpp-httplib` v0.14.x exposes `Result::error()` returning an `Error` enum with `Connection` / `Read` / `Success` etc. | Pitfall 6 ECONNREFUSED classification | Confirmed against in-tree httplib.h (v0.14.3, also v0.20.1 — API is stable). Surface already used by Result truthiness check at `vr_input.cpp:147`. |
| A7 | All P8 success criteria can be validated without visual UAT *except* the "live driver-loaded indicator" + "level meter" + "device-disappeared indicator" UX checks (D-27 #3, #6 visually, #7) | Validation Architecture | Headless tests cover JSON shapes, validation rejection, single-writer grep, localhost bind, and SVR-05 lint. Visual UAT handles the indicators that depend on ImGui rendering on a live driver/SteamVR. |
...
| A9 | The `apps/micmap` UI panel layout has room for a new driver-health pane without redesign — current `renderUI()` at `main.cpp:533` already has separated sections (Status, Audio Device, ...) and a new section is purely additive | Client UI scope (D-11) | Read of `main.cpp:533+` confirms section-by-section layout with `ImGui::Separator()` dividers. Adding a "Driver Health" section is mechanical. |
| A10 | The cpp-httplib v0.20.1 build flags currently set in driver/CMakeLists.txt (`CPPHTTPLIB_OPENSSL_SUPPORT` not defined; `CPPHTTPLIB_NO_EXCEPTIONS` per INTEGRATIONS.md) remain valid in v0.20.1 | cpp-httplib bump | Verifiable by reading the v0.20.1 README; if a new define landed (e.g., `CPPHTTPLIB_USE_BROTLI`) it's opt-in. |
...
|----------|-------------|------------------|
| **Stored data** | `%APPDATA%\MicMap\config.json` already exists; schema unchanged. Driver Init reads it; no migration needed. `training_data.bin` ownership transfer is **P9, not P8**. | None for config.json (read-compatible). For training_data.bin: deferred to P9. |
...
| nlohmann/json v3.11.2 | Driver TUs only (D-01) | ✓ | (existing pin in `external/CMakeLists.txt:10`) | — |
| ImGui v1.90.1 + D3D11 | Client UI driver-health pane | ✓ | (existing pin in `external/CMakeLists.txt:81`) | — |
| WASAPI / `IMMDeviceEnumerator` | `GET /devices` enumerator | ✓ | Windows SDK (existing) | — |
| Win32 `SHGetFolderPathW(CSIDL_APPDATA)` | Log file paths | ✓ | (existing usage at `config_manager.cpp:44`) | — |
| Win32 `ReplaceFileW` | PUT /settings persist | ✓ | (existing usage at `config_manager.cpp:357`) | — |
...
| IPC-04 | `PUT /settings` happy path: validate → swap → persist; round-trip via `GET /settings` | integration (round-trip) | `ctest -R DriverPutSettingsRoundTrip` | ❌ Wave 0 |
...
| HEALTH-01 | Client maps `httplib::Error::Connection` → red, `Success` → green | unit (mock httplib::Result) | `ctest -R ClientDriverLoadedIndicator` | ❌ Wave 0 |
...
- [ ] `tests/test_client_driver_loaded_indicator.cpp` — covers HEALTH-01 ECONNREFUSED classification
...
2. **`GET /telemetry/level` schema — `rms` only, or include `dBFS` and other fields?**
   ...
   - Recommendation: Return both `rms` (linear, [0..1]) and `dBFS` (computed server-side: `20*log10(rms)`, clamped to -120 dB floor). Single source of truth on the server. No peak in v1.6 (HEALTH-D2 trigger sparkline is deferred).
...
- `.planning/phases/08-ipc-contract-reshape/08-CONTEXT.md` — locked decisions D-01..D-30, plan rough structure, UAT regimen
- `.planning/phases/08-ipc-contract-reshape/08-DISCUSSION-LOG.md` — rationale for the four headline calls
- `.planning/REQUIREMENTS.md` — IPC-01..08, HEALTH-01..07, LIB-04 surface specs
- `.planning/ROADMAP.md` §"Phase 8" — six exit criteria, dependency graph
- `.planning/research/SUMMARY.md` — recommended stack, deltas, IPC reshape semantics
...
- `.planning/codebase/INTEGRATIONS.md` — cpp-httplib v0.14.3 in-tree pin, port range, localhost binding
- `.planning/codebase/STRUCTURE.md` — directory layout, `driver/src/`, `src/common/`, `apps/micmap/`
- `driver/src/http_server.{hpp,cpp}` — existing HTTP server, ctor signature, `/health` + `/port` + `/status` + `POST /button` routes; `driverDetectionActiveGetter` callback pattern (P7 D-09)
- `driver/src/device_provider.{hpp,cpp}` — existing Init/Cleanup ordering; AudioWorker + DetectionRunner construction
- `driver/src/audio_worker.{hpp,cpp}` — `state_->rms_logs_emitted` budget pattern (`audio_worker.cpp:325-330`); `state_->sample_rate` atomic (`audio_worker.hpp:124`); WASAPI device enumeration (`audio_worker.cpp:221`); RMS computation (`audio_worker.cpp:317-324`)
...
- `src/steamvr/include/micmap/steamvr/vr_input.hpp` + `src/steamvr/src/vr_input.cpp` — current `IDriverClient` interface; `DriverClient` impl with `connect()`, `tap()`, `getStatus()`, `isDriverDetectionActive()` (P7 D-10), error classification at `vr_input.cpp:147-150`
- `src/common/include/micmap/common/logger.hpp` + `src/common/src/logger.cpp` — `ILogger` interface; `Logger::setLogger()` global access (`logger.hpp:74-120`); default `ConsoleLogger` (`logger.cpp:63`)
- `src/core/include/micmap/core/config_manager.hpp` + `src/core/src/config_manager.cpp` — AppConfig schema; `writeAtomicWindows` ReplaceFileW helper (`config_manager.cpp:322`); `backupAndRotate` corruption rotation (`config_manager.cpp:280`); UTF-8 wstring boundary
- `apps/micmap/main.cpp` — `MicMapApp::initialize()` at line 220; `loadDefault()` at line 222; `saveDefault()` at line 498 (TO BE DELETED at P8 cutover); `onTrigger()` at line 515; `renderUI()` at line 533; existing health indicator pattern at line 542
...
- `external/CMakeLists.txt` — cpp-httplib v0.14.3 pin (`external/CMakeLists.txt:62`), nlohmann_json v3.11.2 pin
...
- Standard stack: HIGH — cpp-httplib v0.20.1 tag confirmed; nlohmann v3.11.2, ImGui, KissFFT, OpenVR all unchanged from prior phases.
- Architecture: HIGH — every named symbol verified against current branch (commit `577d40f docs(state)` at HEAD on `hmd-button`).
- Pitfalls: HIGH — anchored in v1.5/P5/P6/P7 shipped pitfalls + project PITFALLS.md catalog.
