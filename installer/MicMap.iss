...
; ---------------------------------------------------------------
; Phase 10 / INST-09 / D-18: single-version source-of-truth via configure_file().
; installer/version.iss is generated by CMake from installer/version.iss.in at
; configure time; the include below populates {#MICMAP_VERSION} from
; cmake/version.cmake (the SSoT). ISPP resolves "version.iss" relative to this
; file's directory; the absolute-path ISCC invocation in root CMakeLists.txt
; (Pitfall 5 mitigation) ensures that resolution is unambiguous.
;
; The #ifndef MICMAP_VERSION fallback below stays as a safety net for hand-runs
; without configure_file() (e.g., devs running ISCC directly without the cmake
; step). When the include succeeds, MICMAP_VERSION is already defined and the
; #ifndef block is a no-op.
; ---------------------------------------------------------------
#include "version.iss"
...
; Phase 10 / HEALTH-08 / Pitfall 8: tray glyph icons must be present at
; install time or LoadImageW returns nullptr at runtime (apps/micmap/src/
; tray_glyph.cpp::initTrayIcons resolves these via GetModuleFileNameW +
; PathRemoveFileSpecW + PathCombineW, anchored to micmap.exe's directory).
; The 10-02 CMake install(FILES ... DESTINATION bin/resources) rule
; populates {stage}\bin\resources\ at `cmake --install` time; this [Files]
; entry copies the staged .ico assets into {app}\bin\resources\ at install.
Source: "{#STAGE_DIR}\bin\resources\tray_armed.ico"; \
  DestDir: "{app}\bin\resources"; \
  Flags: ignoreversion
Source: "{#STAGE_DIR}\bin\resources\tray_triggered.ico"; \
  DestDir: "{app}\bin\resources"; \
  Flags: ignoreversion
Source: "{#STAGE_DIR}\bin\resources\tray_error.ico"; \
  DestDir: "{app}\bin\resources"; \
  Flags: ignoreversion
